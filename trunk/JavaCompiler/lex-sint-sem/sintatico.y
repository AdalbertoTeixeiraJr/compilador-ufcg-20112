...
field_or_method_declaration :   primitive_type {
				method_or_field_type = $1;
				} colc_opt {
				array_level_def = $3;
				} field_or_method_declaration_rest
...
colc_opt: OPEN_COLC CLOSE_COLC colc_opt_ {
		$$ = $3+1;
	}
	| /** empty **/ {$$ = 0;};

colc_opt_ : OPEN_COLC CLOSE_COLC colc_opt_ {
		$$ = $3+1;
	}
	| /**empty **/ {$$ = 0;};
...
local_variable_declaration :   field_modifiers_ primitive_type colc_opt_ {
				method_or_field_type = $2;
				array_level_def = $3;
				}
...
variable_initializer : assignment_expression {
				checkEqualsArrayLevel(array_level_def, level_access+local_level);
...
conditional_expression : conditional_or_expression  {$$ = $1;}
		| 	conditional_or_expression {
				checkIsBoolean($1);
			} QUESTION_MARK conditional_expression TWO_POINTS conditional_expression {
				$$ = checkQuestionMarkOperator($4,$6);
			};
	
conditional_or_expression : conditional_and_expression conditional_or_expression_ {
				$$ = checkConditionalAndOrOperator($1,$2);
			};

conditional_or_expression_ : {reg++;} OR_LOGIC  conditional_and_expression {
				reg--;
				...
				conditional_or_expression_ {
				$$ = checkConditionalAndOrOperator($3,$5);
				}
                        | /** empty **/ {$$ = "t_empty";};

conditional_and_expression : inclusive_or_expression conditional_and_expression_ {$$ = checkConditionalAndOrOperator($1,$2);};

conditional_and_expression_ : {reg++;} AND_LOGIC  inclusive_or_expression {
				reg--;
				sprintf(ass_code,"%sSUB R%d, R%d, R%d\n", ass_code, reg, reg, reg+1);
				sprintf(ass_code,"%sJNZ lab%d, R%d\n", ass_code, label);
				sprintf(ass_code,"%sMOV R%d, #1 ; Result is true\n", ass_code, reg);
				sprintf(ass_code,"%sJMP lab%d\n", ass_code, label+1);
				sprintf(ass_code,"%slab%d: MOV R%d, #0; Result is false\nlab%d: ", ass_code, label, label+1);
				label+=2;
				}
...
inclusive_or_expression : exclusive_or_expression  inclusive_or_expression_ {
				$$ = checkBitwiseLogicalOperator($1,$2);
				};

inclusive_or_expression_ : {reg++;} OR  exclusive_or_expression { 
				reg--;
				sprintf(ass_code,"%sOR R%d, R%d, R%d\n", ass_code, reg, reg, reg+1);
				} 
				inclusive_or_expression_ {
				$$ = checkBitwiseLogicalOperator($3,$5);
				}
                        | /** empty **/ {$$ = "t_empty";};

exclusive_or_expression : and_expression exclusive_or_expression_ {$$ = checkBitwiseLogicalOperator($1,$2);};

exclusive_or_expression_ :{reg++;} OR_EXC  and_expression { 
				reg--;
...
and_expression_ : {reg++;} AND equality_expression { 
				reg--;
...
equality_expression_ : {reg++;} EQUALOP {strcpy(equalop,yytext);}  relational_expression {
				reg--;
				if(strcmp(equalop,"==")==0){
					sprintf(ass_code,"%sSUB R%d, R%d, R%d\n",ass_code, reg, reg, reg+1);
					sprintf(ass_code,"%sJNZ lab%d, R%d\n",ass_code, label, reg);
				...
				$$ = checkEqualityOperator($4,$6);
				}
                        | /** empty **/ {$$ = "t_empty";};

relational_expression : shift_expression relational_expression_ {$$ = checkRelationalOperator($1,$2);};

relational_expression_ : {reg++;} RELOP {strcpy(relop,yytext);} shift_expression {
				reg--;
				if(strcmp(relop,">=")==0){
...
additive_expression_ : {reg++;} PLUS   multiplicative_expression { 
					reg--; 
					sprintf(ass_code, "%sADD R%d, R%d, R%d\n", ass_code, reg, reg, reg+1 );
					}			
					...
					$$ = checkBinaryExpressionResultType($3,$5);
                        ...
                        | {reg++;} MINUS  multiplicative_expression {
					reg--;
					sprintf(ass_code, "%sSUB R%d, R%d, R%d\n", ass_code, reg, reg, reg+1);
					}
					additive_expression_ {					
...
multiplicative_expression :  unary_expression multiplicative_expression_ {
					$$ = checkBinaryExpressionResultType($1,$2);
				};

multiplicative_expression_ : { reg++;} MULT  unary_expression {
					reg--;
					sprintf(ass_code, "%sMUL R%d, R%d, R%d\n", ass_code, reg, reg, reg+1 );
					} multiplicative_expression_ {
					$$ = checkBinaryExpressionResultType($3,$5);
					}
                        | { reg++;} DIV  unary_expression {
					reg--;
					sprintf(ass_code, "%sDIV R%d, R%d, R%d\n", ass_code, reg, reg, reg+1 );
					} multiplicative_expression_ {
					$$ = checkBinaryExpressionResultType($3,$5);
					}
                        | { reg++;} MOD unary_expression  {
					reg--;
...
method_invocation : 	identifier {
				addCalledMethod($1);reg_count = reg;
				method_name = $1;
			} OPEN_PAREN argument_list CLOSE_PAREN {
				$$ = checkMethodConversion();
			}
		|	field_access OPEN_PAREN {
				addCalledMethod($1);
				reg_count = reg;
				method_name = $1;
			} argument_list CLOSE_PAREN {
				$$ = checkMethodConversion();
			} ;

argument_list : 	assignment_expression {
				addArgsToCalledMethod($1,level_access);
				reg++;
			}
			argument_list_
		|	argument_list_;

argument_list_ : 	VIRGULA assignment_expression {
				addArgsToCalledMethod($2,level_access);
					reg++;
			} argument_list_ 
                | 	/** empty **/ {
				sprintf(ass_code,"%sinvoke %s",ass_code, method_name); 
...
array_creation_expression : NEW primitive_type {
			checkEqualsTypeval(method_or_field_type, $2);
			} dim_expr  dim_exprs {
			checkEqualsArrayLevel(1+$5+local_level,array_level_def);
			};

dim_exprs : OPEN_COLC dim_expr_or_empty CLOSE_COLC dim_exprs {$$ = $4+1;}
	|	/** empty **/  {$$ = 0;};

dim_expr: OPEN_COLC assignment_expression {
		checkArrayCreationExpression($2);
	} CLOSE_COLC;

dim_expr_or_empty : assignment_expression {
			checkArrayCreationExpression($1);
		}
		|	/** empty **/;

left_hand_side :	array_access {
				$$ = getVarTypevalInBothContexts($1);
				local_level = local_level+getVarArrayLevelInGlobalContext($1);
			};

array_access : identifier dim_expr dim_expr_opt {
				local_level = $3+1; 
				level_access = $3+1;
				level_access = isVarFinalInBothContexts($1);
			}
		|	field_access dim_expr dim_expr_opt {
				local_level = $3+1; 
				level_access = $3+1;
				level_access = isVarFinalInGlobalContext($1);
			};
...
statement_expression :          incr_decrement_expression {
					inside_expr = 0;
				}
...
switch_statement : SWITCH OPEN_PAREN assignment_expression {
			checkIsSwitchExpression($3);
			switch_type = $3;
			reg++;
		} CLOSE_PAREN switch_block {
			reg--;
			label = 100+label-label%100;
			sprintf(ass_code, "%slab%d: ", ass_code, label);
		};
...
switch_label : CASE {
			...
			checkEqualsTypeval($3,switch_type);
...
do_statement : DO {
		...
		statement WHILE OPEN_PAREN assignment_expression {
			...
			label = 100+(label-label%100);
		} CLOSE_PAREN PT_VIRGULA;

break_statement : BREAK {
			sprintf(ass_code, "%sJMP lab%d\n", ass_code, label-(label%100)+100);
		} PT_VIRGULA;

continue_statement : CONTINUE {
			sprintf(ass_code, "%sJMP lab%d\n", ass_code, label-(label%100));
		} PT_VIRGULA;

goto_statement : GOTO identifier {
			checkLabelInCurrMethod($2);
			sprintf(ass_code,"%sJMP %s\n",ass_code, $2);
		} PT_VIRGULA;

return_statement : RETURN expression_opt_return {
			checkReturnTypeAndLevelInCurrMethod($2, level_access+local_level);
			has_return++;
		} PT_VIRGULA;
